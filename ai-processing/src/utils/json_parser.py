...
def extract_json_from_text(text: str) -> Optional[Union[dict, list]]:
    ...
    looks_like_missing_brace = (
        not trimmed_text.startswith("{") and 
        not trimmed_text.startswith("[") and
        "}" in trimmed_text and 
        '"' in trimmed_text
    )
    ...
    if looks_like_missing_brace or ("}" in trimmed_text and '"' in trimmed_text and not trimmed_text.startswith("{")):
        ...
        if first_key_match:
            ...
            if last_brace_after_start != -1:
                ...
                potential_content = trimmed_text[max(0, start_pos - 1):last_brace_after_start + 1].strip()
        ...
        first_key_match = re.search(r'["\']([^"\']+)["\']\s*:', trimmed_text, re.MULTILINE | re.DOTALL)
        last_brace = trimmed_text.rfind("}")
