  ...
  describe('Response Schema Validation', () => {
    it('should return 201 with correct schema when request is valid', async () => {
      ...
      if (response.status === 201) {
        ...
        expect(
          new Date(response.body.estimated_completion_time).getTime()
        ).toBeGreaterThan(0);
      }
    });
    ...
    it('should return 429 when rate limit is exceeded', async () => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (i < 3) {
          ...
          // 4th request should return 429 (or 500 if services not configured)
          expect([429, 404, 500, 501]).toContain(response.status);
